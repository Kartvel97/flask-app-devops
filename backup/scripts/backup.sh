...
# TODO: handle this better if docker not running
...
if command -v aws >/dev/null 2>&1; then
    ...
    if [ -n "${S3_BUCKET}" ]; then
        ...
        aws s3 cp "${BACKUP_FILE}" "s3://${S3_BUCKET}/backups/${BACKUP_NAME}.tar.gz" 2>/dev/null || echo "S3 upload failed"
    else
...
# TODO: handle this better - maybe keep last N backups instead of by days
